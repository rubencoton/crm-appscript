...
function replaceOnce(search, replace, label) {
  if (!text.includes(search)) throw new Error('No se encontro: ' + label);
...
`;
  if (!text.includes(anchor)) throw new Error('No se encontro ancla para auditor extremo');
