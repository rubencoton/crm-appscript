...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('1) Configurar clave API y password', 'configurarSistema')
    .addSeparator()
    .addItem('2) Iniciar escaner total', 'lanzarModoTotal')
    .addItem('3) Auditar matriz (solo CONCURSOS)', 'lanzarModoActualizar')
    .addItem('4) Analizar NUEVOS + Radar', 'lanzarModoNuevas')
    .addItem('5) Continuar escaner (si hubo pausa)', 'continuarEscaner')
    .addSeparator()
    .addItem('6) Enviar boletin a BANDAS', 'verificarYEnviarCorreos')
    .addSeparator()
    .addItem('7) Parar proceso', 'solicitarParada')
...
function configurarSistema() {
  ...
  const apiPrompt = ui.prompt(
    'Configuracion',
    'Pega tu GEMINI_API_KEY (se guarda en Script Properties):',
  ...
  const passPrompt = ui.prompt(
    'Configuracion',
    'Define la password interna del CRM:',
    ui.ButtonSet.OK_CANCEL
  );
  ...
  if (!apiKey || !password) {
    ui.alert('Faltan datos. No se guardo la configuracion.');
    return;
  }

  props.setProperty('GEMINI_API_KEY', apiKey);
  props.setProperty('CRM_PASSWORD', password);
  ui.alert('Configuracion guardada correctamente.');
}
...
function continuarEscaner() {
  const props = PropertiesService.getScriptProperties();
  const mode = props.getProperty('RUN_MODE');
  if (!mode) {
    SpreadsheetApp.getUi().alert('No hay ejecucion pausada.');
    return;
  }
  ejecutarConPassword_(mode, true);
}
...
function getApiKey_() {
  ...
  if (!key) {
    throw new Error('Falta GEMINI_API_KEY. Ejecuta "Configurar clave API y password".');
  }
  return key;
}
...
function verificarYEnviarCorreos() {
  const ui = SpreadsheetApp.getUi();
  const props = PropertiesService.getScriptProperties();
  const pass = (props.getProperty('CRM_PASSWORD') || '').trim();
  if (!pass) {
    ui.alert('Primero ejecuta "Configurar clave API y password".');
    return;
  }

  const p = ui.prompt('Seguridad', 'Introduce password para enviar boletin:', ui.ButtonSet.OK_CANCEL);
  if (p.getSelectedButton() !== ui.Button.OK) return;
  if ((p.getResponseText() || '').trim() !== pass) {
    ui.alert('Password incorrecta.');
    return;
  }
  ...
  const ok = ui.alert(
    'Confirmar envio',
    'Se enviara el boletin a todas las bandas con email valido usando solo concursos "ABIERTA". Continuar?',
    ui.ButtonSet.YES_NO
  );
